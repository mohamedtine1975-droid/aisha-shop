...
if (cursor && window.innerWidth > 768) {
  ...
  document.querySelectorAll('a, button, .product-card, .cat-pill').forEach(el => {
    el.addEventListener('mouseenter', () => {
      ...
      follower.style.borderColor = 'var(--wine-light)';
    });
...
(function initParticles() {
  ...
  class Particle {
    ...
    draw() {
      ...
      ctx.fillStyle = `rgba(92,26,46,${this.opacity})`;
      ctx.fill();
    }
  }
...
function renderProducts() {
  ...
  if (!list.length) {
    grid.innerHTML = `
      <div style="grid-column:1/-1;text-align:center;padding:4rem 2rem;color:var(--muted)">
        ...
        <p style="font-size:0.85rem;margin-top:0.5rem">Essayez une autre catégorie ou terme de recherche</p>
      </div>`;
    return;
  }
  ...
  grid.innerHTML = list.map(p => {
    ...
    return `
    <div class="product-card reveal" onclick="openProductModal(${p.id})" style="transition-delay:${Math.random()*0.2}s">
      <div class="product-img-wrap">
        ...
        <div class="product-overlay">
          <button class="overlay-btn" onclick="toggleWish(event,${p.id})" title="${inWish?'Retirer des favoris':'Ajouter aux favoris'}">${inWish?'❤️':'🤍'}</button>
          <button class="overlay-btn" onclick="openProductModal(${p.id})" title="Voir le produit">👁️</button>
          <button class="overlay-btn" onclick="quickAdd(event,${p.id})" title="Ajouter au panier">🛍️</button>
        </div>
      </div>
      <div class="product-body">
        ...
        <div class="product-foot">
          ...
          <button class="add-btn" onclick="quickAdd(event,${p.id})" title="Ajouter au panier">+</button>
        </div>
      </div>
    </div>`;
  }).join('');
...
function quickAdd(e, id) {
  e.stopPropagation();
  addToCart(Store.products.find(p => p.id === id));
}
...
function removeCartItem(id, size) {
  ...
  if (idx !== -1) { Store.cart.splice(idx, 1); Store.save(); updateCartUI(); }
}
...
function updateCartUI() {
  ...
  const waText = 'Bonjour Aïsha-T Shop ! Voici ma commande :\n' +
...
function openProductModal(id) {
  ...
  // reset sizes
  document.querySelectorAll('.size-chip').forEach((el, i) => {
...
function getSelectedSize() {
  return document.querySelector('.size-chip.active')?.textContent || 'M';
}

function addCurrentToCart() {
  if (Store.currentProduct) {
    addToCart(Store.currentProduct, getSelectedSize());
    closeProductModal();
    toggleCart();
  }
}
...
window.addEventListener('load', async () => {
  ...
  // Charge les produits depuis le cloud en parallèle
